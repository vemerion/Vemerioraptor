...
public class VemerioraptorModel extends EntityModel<VemerioraptorEntity> {
	...
	public VemerioraptorModel() {
		...
		this.setRotateAngle(neck1, -0.503F, 0.0F, 0.0F);
...
//        bipedRightArm.rotateAngleX = MathHelper.cos(limbSwing * 0.6662f + (float) Math.PI) * 2f * limbSwingAmount * 0.5f;
	...
	public void setLivingAnimations(VemerioraptorEntity entityIn, float limbSwing, float limbSwingAmount,
			float partialTick) {
		armRotations(entityIn, limbSwing, limbSwingAmount, partialTick);
	}

	private void armRotations(VemerioraptorEntity entityIn, float limbSwing, float limbSwingAmount, float partialTick) {
		ModelRenderer[] arms = new ModelRenderer[] { leftArm1, leftArm2, leftArm3 };
		float[] armStartRotations = new float[] { 0.45f, -1.1f, 0.47f };
		swingProgress = entityIn.getSwingProgress(partialTick);

		if (!entityIn.isSwingInProgress) {
			for (int i = 0; i < arms.length; i++) {
				arms[i].rotateAngleX = MathHelper.cos(limbSwing * 0.6662f) * 2f * limbSwingAmount * 0.25f
